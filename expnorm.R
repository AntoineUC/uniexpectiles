...
cppFunction(depends="RcppArmadillo",plugins="cpp11",code="
            arma::vec expnorm(arma::vec& alphas, const double mean=0.0, const double sigma=1.0) {
              arma::vec alphabis=((abs((2*alphas)-1.0)*(-1.0))+1)/2.0;
              ...
              while(ecart.max()>0.0000001){
                e1=(1-2*alphabis)%arma::normpdf(e)/((2*alphabis-1)%arma::normcdf(e)-alphabis);
                ecart=abs(e1-e);
                e=e1;
              }
